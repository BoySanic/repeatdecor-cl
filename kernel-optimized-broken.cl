...
__kernel void searchKernel(
    ulong        start_seed,    // (0) absolute start of this sub–batch
    ulong        nValidSeeds,   // (1) number of real work-items
    __global Result*         results,     // (2)
    volatile __global int*   result_idx,  // (3)
    volatile __global uint*  checksum     // (4)
){
    // get_global_size(0) may be > nValidSeeds due to padding–round-up
    ...
    for(int i=0; i<=HASH_BATCH_SIZE; i++)
        hashes[i] = mix64(seed_base + (ulong)i * XL);

    #pragma unroll
    for(int i=0; i<HASH_BATCH_SIZE; i++){
        PRNG128 prng = prng_init2(hashes[i], hashes[i+1]);
        if(!goodLower32(&prng)) continue;
        ulong curr_s = seed_base + (ulong)i * XL;
        processFullPrngState(curr_s, results, result_idx);
        // count *every* goodLower32 pass, as in CUDA
